    ...
    if isinstance(message, collections.Mapping):
        ...
        for key, value in values.items():
            ...
            if field.label == FieldDescriptor.LABEL_REPEATED:
                ...
                dict_to_protobuf(value, getattr(message, key, None))
            else:
...
def _get_type_cast_callable(message, field):
    ...
    if field.type == FieldDescriptor.TYPE_MESSAGE:
        ...
        return lambda message: protobuf_to_dict(message)
        ...
        if isinstance(value, collections.Mapping):
            ...
            for key, val in value.items():
                if _is_type_scalar(val):
                    ...
                    containing_dict[key] = protobuf_to_dict(val)
