   ...
   public:
    ...
    ValueType & operator[](int col) {
      if (!gp->inBounds(row, col)) {
        throw ("Grid index values out of range");
      }
...
void Grid<ValueType>::resize(int nRows, int nCols) {
  if (nRows < 0 || nCols < 0)
    throw(fmt::format("Attempt to resize grid to invalid size ({}, {}", nRows, nCols));
...
ValueType Grid<ValueType>::get(int row, int col) {
  if (!inBounds(row, col)) throw ("get: Grid indices out of bounds");
...
void Grid<ValueType>::set(int row, int col, ValueType value) {
  if (!inBounds(row, col)) throw ("set: Grid indices out of bounds");
